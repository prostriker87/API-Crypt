$(document).on('submit','#form_2crypt',function(event){
	...
	$.ajax({
		url: 'https://taldiscom.es/API-Crypt/remote/API-crypto-test.php?2crypt',
		type: 'POST',
        data: fd,
        processData: false,
        contentType: false,
	...
	.done(function(respuesta) {
		...
		if(!respuesta.info.error){
			...
			$('.show').show();
			var out_debug = Base64.decode(respuesta.output.debug);
			$('#debug_2crypt').html(out_debug);
			for (let index = 0; index < respuesta.output.data.length; ++index) {
				let value = respuesta.output.data[index];
			$('.debug_'+index).html(Base64.decode(value));
			}
...
$(document).on('submit','#form_2decrypt',function(event){
	...
	$.ajax({
		url: 'https://taldiscom.es/API-Crypt/remote/API-crypto-test.php?2decrypt',
		type: 'POST',
        data: fd,
        processData: false,
        contentType: false,
	...
	.done(function(respuesta) {
		...
		if(!respuesta.info.error){
			...
			$('.2decrypt').show();
			var out_debug = Base64.decode(respuesta.output.debug);
			$('#debug_2decrypt').html(out_debug);
			for (let index = 0; index < respuesta.output.data.length; ++index) {
				let value = Base64.decode(respuesta.output.data[index]);
			$('.debug_2decrypt_'+index).html(value);
			}
...
$(document).ready(function(){
	//Botones debug encriptado
	$(".button_debug_2crypt_on").bind('click', function(){
				$(".button_debug_2crypt_on").hide(); 
				$("#debug_2crypt").show();
				$(".button_debug_2crypt_off").show(); 
		}); 
	$(".button_debug_2crypt_off").bind('click', function(){
				$(".button_debug_2crypt_off").hide();  
				$("#debug_2crypt").hide();
				$(".button_debug_2crypt_on").show(); 
		});
	//Botones debug desencriptado
	$(".button_debug_2decrypt_on").bind('click', function(){
				$(".button_debug_2decrypt_on").hide(); 
				$("#debug_2decrypt").show();
				$(".button_debug_2decrypt_off").show(); 
		}); 
	$(".button_debug_2decrypt_off").bind('click', function(){
				$(".button_debug_2decrypt_off").hide();  
				$("#debug_2decrypt").hide();
				$(".button_debug_2decrypt_on").show(); 
		}); 
